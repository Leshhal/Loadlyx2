...
const customerNavigation = [['Overview','/app/dashboard','chart'],['My loads','/loads','route'],['Offers','/app/marketplace-offers','spark'],['Agreements & payments','/app/marketplace-transactions','chart'],['Post a load','/quote','spark'],['Marketplace','/loadboard','search'],['Store','/catalog','store']];
const brokerNavigation = [['Overview','/app/dashboard','chart'],['CRM','/app/crm','users'],['Customer loads','/loads','route'],['Offers','/app/marketplace-offers','spark'],['Agreements & payouts','/app/marketplace-transactions','chart'],['Load board','/loadboard','search'],['Provider & payouts','/app/provider-onboarding','check'],['Quotes','/app/quotes','chart'],['Marketplace store','/catalog','store']];
const carrierNavigation = [['Overview','/app/dashboard','chart'],['Available loads','/loadboard','search'],['My loads','/loads','route'],['Offers','/app/marketplace-offers','spark'],['Agreements & payouts','/app/marketplace-transactions','chart'],['Provider & payouts','/app/provider-onboarding','check'],['Dispatch view','/app/dispatch','chart'],['Carrier profile','/carriers/signup','users']];
...
export default function AppShell({ mode = 'tenant', children }) {
  ...
  const links = useMemo(() => { if (mode === 'admin') return role === 'TENANT_ADMIN' ? tenantAdminNavigation : platformAdminNavigation; if (role === 'MARKETPLACE_USER') return customerNavigation; if (role === 'BROKER') return brokerNavigation; if (role === 'CARRIER') return carrierNavigation; return tenantNavigation; }, [mode, role]);
  const current = links.find(([, href]) => { const route = href.split('#')[0]; return pathname === route || pathname.startsWith(`${route}/`); })?.[0] || 'Workspace';
  const workspaceLabel = mode === 'admin' ? 'Platform administration' : role === 'MARKETPLACE_USER' ? 'Loadboard workspace' : role === 'BROKER' ? 'Broker workspace' : role === 'CARRIER' ? 'Carrier workspace' : 'Tenant workspace';
