...
const loadSchema = z.object({ title: z.string().min(4).max(160), description: z.string().min(20).max(5000), category: z.string().max(120).optional(), originCity: z.string().min(2).max(120), originRegion: z.string().max(120).optional(), originCountry: z.string().length(2), destinationCity: z.string().min(2).max(120), destinationRegion: z.string().max(120).optional(), destinationCountry: z.string().length(2), pickupDate: z.coerce.date(), deliveryDate: z.coerce.date().optional(), equipmentType: z.string().max(120).optional(), trailerType: z.string().max(120).optional(), weightKg: z.number().positive().optional(), distanceKm: z.number().positive().optional(), fullOrPartial: z.enum(['FULL','PARTIAL']).default('FULL'), hazmat: z.boolean().default(false), teamDriverRequired: z.boolean().default(false), volumeCubicM: z.number().positive().optional(), dimensionsJson: z.record(z.any()).optional(), specialHandling: z.string().max(1000).optional(), imageUrls: z.array(z.string().url()).max(12).optional(), documentUrls: z.array(z.string().url()).max(12).optional(), currency: z.string().length(3).default('cad'), visibility: z.enum(['PUBLIC','PRIVATE','INVITE_ONLY']).default('PUBLIC'), expiresAt: z.coerce.date().optional(), budgetCents: z.number().int().positive().optional(), instantBookEnabled: z.boolean().default(false), instantBookCents: z.number().int().positive().optional(), instantBookRules: z.record(z.any()).optional(), postedForCustomer: z.boolean().default(false), customerName: z.string().max(200).optional(), customerEmail: z.string().email().optional() }).superRefine((value, ctx) => { if (value.instantBookEnabled && !value.instantBookCents) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['instantBookCents'], message: 'Instant-book price is required' }); });
...
router.get('/loads', optionalAuth, async (req, res, next) => {
  try {
    ...
    const simulationEvents = globalSimulation?.enabled
      ...
      : [];
    const presented = presentMarketplaceLoads({ loads, simulationEvents, authenticated: Boolean(req.user) });
    const sort = String(q.sort || 'earliest_pickup');
    if (sort === 'newest') presented.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
    if (sort === 'highest_rate') presented.sort((a,b) => (b.budgetCents || 0) - (a.budgetCents || 0));
